...
export default function Home() {
  ...
  const activeSubject =
    currentSyllabus?.subjects.find((s) => s.id === selectedSubjectId) ||
  ...
  const getSubjectColor = (subjectName: string) => {
    switch (subjectName.toLowerCase()) {
      case "physics":
        return "text-[#00f0ff]"; // Electric Cyan
      case "chemistry":
        return "text-[#b500ff]"; // Neon Purple
      case "mathematics":
        return "text-[#ff5e00]"; // Bright Orange
      default:
        return "text-white";
    }
  };
  ...
  const getSubjectHoverBorder = (subjectName: string) => {
    switch (subjectName.toLowerCase()) {
      case "physics":
        return "hover:border-[#00f0ff] hover:text-[#00f0ff]";
      case "chemistry":
        return "hover:border-[#b500ff] hover:text-[#b500ff]";
      case "mathematics":
        return "hover:border-[#ff5e00] hover:text-[#ff5e00]";
      default:
        return "hover:border-white hover:text-white";
    }
  };
  ...
  const getSubjectActiveBg = (subjectName: string) => {
    switch (subjectName.toLowerCase()) {
      case "physics":
        return "bg-[#00f0ff] border-[#00f0ff]";
      case "chemistry":
        return "bg-[#b500ff] border-[#b500ff]";
      case "mathematics":
        return "bg-[#ff5e00] border-[#ff5e00]";
      default:
  ...
  const getSubjectActiveTextBorder = (subjectName: string) => {
    switch (subjectName.toLowerCase()) {
      case "physics":
        return "border-[#00f0ff] text-[#00f0ff] bg-[#00f0ff]/10";
      case "chemistry":
        return "border-[#b500ff] text-[#b500ff] bg-[#b500ff]/10";
      case "mathematics":
        return "border-[#ff5e00] text-[#ff5e00] bg-[#ff5e00]/10";
      default:
  ...
  const getSubjectBorderColor = (subjectName: string) => {
    switch (subjectName.toLowerCase()) {
      ...
      case "chemistry":
        return "border-[#b500ff]";
      case "mathematics":
        return "border-[#ff5e00]";
      default:
        return "border-white";
    }
  };
          ...
          >
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6 border-b border-white/10">
              <div className="flex items-center gap-4">
                ...
                >
                  <span className="border border-white/10 px-2 py-1 rounded-none hover:bg-white/10">
                    ← Back
                  </span>
                </button>
            ...
            {activeSubject && (
              ...
              >
                <div className="flex items-end justify-between pb-4">
                  <h2
                    className={`text-4xl md:text-5xl font-black uppercase tracking-tighter ${getSubjectColor(activeSubject.name)}`}
                  >
                    {activeSubject.name}
                  </h2>
                ...
                <div className="grid gap-2 border-t border-white/10 pt-4">
                  {activeSubject.topics.map((topic, index) => {
                    ...
                    const revs = progress[topic.id]?.revisions || [
                      false,
                      false,
                      false,
                      false,
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          >
                            Done
                            <div
                              className={`w-3 h-3 flex items-center justify-center ${isCompleted ? "text-black" : "hidden"}`}
                            >
      ...
      <AnimatePresence>
        {verificationData.isOpen && (
          ...
          >
            ...
            >
              <div className="flex flex-col gap-2 text-center">
                <h2 className="text-xl font-bold uppercase tracking-widest text-[#ff003c]">
              ...
              <div className="bg-white/10 p-3 text-center border border-white/20">
                <span className="text-2xl font-black tracking-widest select-all">
                  {verificationData.expectedKey}
                </span>
              </div>
              <input
                type="text"
              ...
              <div className="flex gap-4">
                ...
                <button
                  onClick={() => {
                    if (
                      verificationData.input === verificationData.expectedKey
                    ) {
                      if (verificationData.type === "topic") {
                        forceToggleTopic(verificationData.id);
                      } else if (
                        verificationData.type === "revision" &&
                        verificationData.revIndex !== undefined
                      ) {
                        forceToggleRevision(
                          verificationData.id,
                          verificationData.revIndex,
                        );
                      }
                      setVerificationData((prev) => ({
                        ...prev,
